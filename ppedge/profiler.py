...
def resize(img, reference_img, n_channel=3):
    ...
    if len(img.shape) != 3 or len(reference_img.shape) != 3:
        raise ValueError("image shape must be 3 dim, however, input: {}".format(img.shape))
...
class Profiler(object):
    ...
    Parameters
    ----------
    graph: tf.keras.Seqeunce, or tf.keras.model
    training_x: np.array

    '''

    def __init__(self, graph, training_x, last_conv_layer, repeat=150):
        self.graph = graph
        self.repeat = repeat
        self.x = training_x
    ...
    def _get_privacy_img(self, x, cutoff):
        ...
        '''
        if len(x.shape) >= 4:
            raise ValueError("X image shape must be under 4 dims, but given shape : {}".format(x.shape))
    ...
    def get_privacy_profiling(self, batch_size, return_df=True):
        ...
        if return_df:
            layer_names = [layer.name for layer in self.graph.layers[1:self.last_conv_index]]
    ...
    def get_layer_runtime(self, batch_size=100, repeat=150, return_size=True):
        ...
        for index in range(1, len(self.graph.layers)):
            front_model = tf.keras.Model(self.graph.input, self.graph.layers[index - 1].output)
